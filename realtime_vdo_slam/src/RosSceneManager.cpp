...
VDO_SLAM::RosSceneManager::RosSceneManager(ros::NodeHandle& _nh) :
        ...
        listener(tf_buffer)
    {
        ...
        nh.getParam("/ros_vdoslam/odometry_ground_truth_topic", gt_odom_topic);

        if(!gt_odom_topic.empty()) {
            odom_repub_sub = nh.subscribe<nav_msgs::Odometry>(gt_odom_topic, 100, &RosSceneManager::odom_gt_callback, this);
        }
...
void VDO_SLAM::RosSceneManager::odom_gt_callback(const nav_msgs::OdometryConstPtr& msg) {
    if (is_first) {
